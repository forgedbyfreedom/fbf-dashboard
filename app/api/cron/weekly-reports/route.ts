...
export async function GET(request: NextRequest) {
  ...
  for (const client of clients) {
    try {
      const reportData = await generateReportData(client.id, 'weekly')
      ...
      const fileName = `${client.id}/weekly-${reportData.endDate}.pdf`
      await uploadAndArchive({
        ...
        originalName: `weekly-${reportData.endDate}.pdf`,
        stage: 'other',
        clientId: client.id,
        upsert: true,
      })
      ...
      if (client.email && pdfBuffer) {
        try {
          await sendEmail({
            to: client.email,
            subject: 'Your Weekly Progress Report — Forged by Freedom',
            ...
            attachments: [{
              filename: `FBF-weekly-report-${reportData.endDate}.pdf`,
